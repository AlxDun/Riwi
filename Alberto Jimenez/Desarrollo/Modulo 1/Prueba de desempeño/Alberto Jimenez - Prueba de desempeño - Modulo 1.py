inventory={"guineo": [500.0, 60], "coco": [1500.0, 20], "telefono": [800000.0, 10], "monitor": [450000.0, 15], "trapero": [5000.0, 30]} # The dictionary where de products will be saved
...
def addProduct(): # The function where the user can add the products to the inventory with the name, the price and the amount
    while True:
        name=input("Ingrese nombre del producto (0 para volver al menú): ").lower().strip()
        n()
        if name=="0": return
...
def consultProduct(): # The function will display the products in the inventory to consult any of them just writing the name
    ...
    while True:
        ...
        n()
        try:
            name=input("Ingrese el nombre del producto a consultar (0 para volver al menú): ").lower().strip()
            if name=="0": return n()
            print(f"El precio es: ${inventory[name][0]:.2f}"), print(f"La cantidad es: {inventory[name][1]}"), n()
...
def updateProduct(): # When the user wants to update a product just write the name and the new details of the product and then, the product will update
    ...
    while True:
        ...
        for i in inventory.keys(): print(i)
        n()
        name=input("Ingrese el nombre del producto a actualizar (0 para volver al menú): ").lower().strip()
        n()
        if name=="0": return
...
def calculateInventory(): # When the function is called, the inventory total is printed
    ...
    for i in inventory.values():
        ...
        total+=productTotal
    print(f"El total del inventario es: ${total:.2f}"), n()
print("Bienvenido al sitema de gestion de inventario."), n()
while True:
